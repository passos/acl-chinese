...
为什么要学 Lisp？因为它让你能做一些其它语言做不到的事情。如果你只想写一个函数来返回所有小于 ``n`` 的自然数的总和，那么用 Lisp 和 C 是差不多的：
...
如果你只想做这种简单的事情，那用什么语言都不重要。假设你想写一个函数，参数为数字 ``n`` ，返回一个能够把 ``n`` 与自身的传入参数 (argument)相加的新函数。
...
如果你没听过\ `词法闭包 「Lexical Closure」 <http://zh.wikipedia.org/zh-cn/%E9%97%AD%E5%8C%85_(%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%A7%91%E5%AD%A6))>`_  (上述  ``addn``  的范例)，相信我， Lisp 程序员一直在使用它。无论程序代码有多长，很难找到一个 Common Lisp 程序没有用到闭包的好处。到 112 页时，你就能学会用它了。
...
有了宏、闭包以及运行期类型，Lisp 凌驾在面向对象程序设计之上。如果你能偶理解上面那句话，也许你无需阅读本书。你得充分了解 Lisp 才能明白为什么此言不虚。这不仅仅是说说而已，而是很重要的一点，在 17 章会用程序相当明确的证明这一点。

第二章到第十三章会循序渐进地介绍所有你需要理解第 17 章程序的概念。你的努力会有所回报：你会感到在 C++ 编程是窒碍难行的，就像有经验的 C++ 程序员用 Basic 编程会感到窒息一样。如果我们想想这种感觉从何而来，也许会更加令人振奋。 编写 Basic 对于平常用 C++ 编程是令人感到窒息的，是因为有经验的 C++ 程序员知道一些用 Basic 不可能表达出来的技术。同样地，学习 Lisp 不仅教你学会一门新的语言 ── 它教你崭新的并且更强大的程序思考方法。
